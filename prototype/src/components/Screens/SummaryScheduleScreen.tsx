...
export const SummaryScheduleScreen: React.FC<SummaryScheduleScreenProps> = ({
  ...
  // Fetch summary for current month when history view navigates (standalone mock)
  const fetchMonth = useCallback(async (_date: Date) => {
      ...
      .forEach((s) => {
        (s.assignedCTVs || []).forEach((ctv) => {
          ...
          if (!uniqueMap.has(key)) {
            uniqueMap.set(key, {
              ...ctv,
              room: ctv.room || s.room || account?.workRoom || account?.room || "Chưa cập nhật",
