...
export const envSchema = z
  .object({
    ...
    SNAPSHOT_TRACKING_START_DATE: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/)
      .optional(),
    RATE_LIMIT_KEY_SECRET: z
      .string()
      .min(16)
      .default('development-default-rate-limit-secret-32-chars-long'),
    TRUSTED_PROXY_IPS: z.string().default(''),
    E2E_TEST: z.preprocess((val) => val === 'true' || val === true, z.boolean()).default(false),
  })
