...
vi.mock('../src/app.js', () => ({
  createApp: () => ({
    listen: () => ({ close: mocks.close, closeIdleConnections: mocks.closeIdleConnections }),
  }),
}));
vi.mock('../src/jobs/schedule-snapshot.job.js', () => ({
  startScheduleSnapshotJob: () => ({ stop: mocks.stop }),
}));
...
vi.mock('../src/modules/health/health.controller.js', () => ({
