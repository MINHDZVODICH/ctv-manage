...
function harness() {
  ...
  const tx = {
    ...
    $queryRaw: vi.fn(async (sql: TemplateStringsArray) => {
      ...
      if (query.includes('WorkHistoryProgress'))
        return [
          { trackingStartDate: new Date('2026-09-07'), lastProcessedDate: new Date('2026-09-06') },
        ];
      if (query.includes('SnapshotRun'))
        return [
          {
            id: 'run',
            status: 'RUNNING',
  ...
  return {
    ...
    readSources,
    commit,
    coordinator: new SnapshotCoordinatorService(db as unknown as PrismaClient),
  };
}
...
describe('Snapshot clock and commit boundaries', () => {
  ...
  it('emits success only after commit resolves', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith(
      expect.objectContaining({ event: 'snapshot.succeeded' }),
      expect.any(String),
    );
  });
  it('does not emit success when commit fails', async () => {
    ...
    await expect(coordinator.executeAttempt('2026-09-07', 'owner')).rejects.toThrow(
      'COMMIT_FAILED',
    );
    expect(logger.info).not.toHaveBeenCalled();
  });
