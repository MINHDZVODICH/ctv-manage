...
import { messageFor, type AccountDetail, type AccountStatus, type AccountSummary, type FileCategory, useAccounts } from './useAccounts';
...
export function AccountListScreen() {
  ...
  const mutate = async (operation: () => Promise<unknown>, success: string) => {
    ...
    catch (reason) { setDialogError(messageFor(reason)); }
  ...
  return <section className="accounts-workspace" aria-labelledby="accounts-title">
    ...
    {(detailLoading || selected || dialogError) && <ViewAccountDetailModal account={selected} isLoading={detailLoading} error={dialogError} isSaving={isMutating} onClose={() => { setSelected(null); setDialogError(null); }} onSaveNotes={(notes) => selected && void mutate(async () => setSelected(await accounts.saveNotes(selected, notes)), 'Đã lưu ghi chú.')} onResetPassword={() => selected && setResetAccount(selected)} onReplaceFile={replaceFile} onDeleteFile={deleteFile} />}
    {resetAccount && <ResetPasswordModal account={resetAccount} isSubmitting={isMutating} error={dialogError} onClose={() => setResetAccount(null)} onConfirm={(newPassword, requireChange) => void mutate(async () => { await accounts.resetPassword(resetAccount.id, newPassword, requireChange, globalThis.crypto?.randomUUID?.() ?? `reset-${Date.now()}`); setResetAccount(null); }, 'Đã đặt lại mật khẩu tài khoản.')} />}
  </section>;
}
