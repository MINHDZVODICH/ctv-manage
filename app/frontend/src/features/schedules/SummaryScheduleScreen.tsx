...
export function SummaryScheduleScreen({ initialMonth = todayInBangkok().slice(0, 7) }: { initialMonth?: string }) {
  ...
  return <section className="feature-screen summary-schedule-screen" aria-labelledby="summary-schedule-title">
    ...
    {(profileLoading || profile || profileError) && <ViewAccountDetailModal account={profile} isLoading={profileLoading} error={profileError} isSaving={false} onClose={() => { setProfile(null); setProfileError(null); }} onSaveNotes={() => undefined} onResetPassword={() => undefined} onEditProfile={() => undefined} onReplaceFile={() => undefined} onDeleteFile={() => undefined} />}
  </section>;
}

function calendarDays(month: string): Array<string | null> { const [year, number] = month.split('-').map(Number); const first = new Date(Date.UTC(year, number - 1, 1)); const last = new Date(Date.UTC(year, number, 0)).getUTCDate(); const days: Array<string | null> = []; let row: Array<string | null> = Array((first.getUTCDay() + 6) % 7).fill(null); for (let day = 1; day <= last; day += 1) { const current = new Date(Date.UTC(year, number - 1, day)); const weekday = current.getUTCDay(); if (weekday >= 1 && weekday <= 5) row.push(`${month}-${String(day).padStart(2, '0')}`); if (weekday === 5) { days.push(...row); row = []; } } if (row.length) { while (row.length < 5) row.push(null); days.push(...row); } return days; }
