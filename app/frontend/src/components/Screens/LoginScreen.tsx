...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess, onRequestRegister }) => {
  ...
  // Login form state
  const [loginEmail, setLoginEmail] = useState("admin@vienkhcn.vn");
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.registrationRequests.create({
        name: regName.trim(),
        email: regEmail.trim(),
        phone: regPhone.trim(),
        dob: `${regDay}/${regMonth}/${regYear}`,
        password: regPassword,
        cccdFrontBase64: cccdFront || undefined,
        cccdBackBase64: cccdBack || undefined,
        cvFileBase64: cvFile || undefined,
      ...
      setRegErrors({ regEmail: err.message || "Đăng ký thất bại. Vui lòng thử lại!" });
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleForgotEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.otp) {
        setOtpNotice(`(Mã OTP thử nghiệm: ${res.otp})`);
      }
      setMode("forgot_otp");
    } catch (err: any) {
      setForgotEmailError(err.message || "Email không tồn tại trong hệ thống!");
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    setIsProcessing(true);
    try {
      const res = await api.auth.verifyOtp(forgotEmail, otpCode);
      onLoginSuccess(res.user);
    } catch (err: any) {
      setOtpError(err.message || "Số bạn đã nhập không khớp với mã. Vui lòng thử lại!");
    } finally {
      setIsProcessing(false);
    }
  };
