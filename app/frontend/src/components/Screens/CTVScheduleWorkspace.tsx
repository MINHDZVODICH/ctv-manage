...
export const CTVScheduleWorkspace: React.FC<CTVScheduleWorkspaceProps> = ({
  ...
  const handleRegisterSchedule = async (event: React.FormEvent) => {
    ...
    for (let i = 0; i < 5; i++) {
      const dayTypes = weeklyPattern[i] || [];
      if (dayTypes.includes("morning")) {
        slots.push({ weekday: i, period: "MORNING", enabled: true });
      }
      if (dayTypes.includes("afternoon")) {
        slots.push({ weekday: i, period: "AFTERNOON", enabled: true });
      }
    }
    ...
    try {
      await api.schedules.saveMyRegistration({
        room,
        startDate,
        endDate,
      ...
      const updatedShifts = await api.schedules.getMyShifts();
      onUpdateShifts(updatedShifts);
      setCalendarDate(rangeStart);
      setCalendarView("week");
      setIsRegistrationOpen(false);
      onShowToast("Đã lưu mẫu lịch làm việc và đồng bộ toàn bộ ca làm.");
    } catch (err: any) {
      onShowToast(err.message || "Lỗi khi lưu lịch làm việc");
    }
  };
  ...
  const handleCancelShift = async () => {
    ...
    try {
      await api.schedules.cancelShift(selectedShift.id, "single");
      const updatedShifts = await api.schedules.getMyShifts();
      onUpdateShifts(updatedShifts);
      setSelectedShift(null);
      onShowToast(
      ...
      onShowToast(err.message || "Lỗi khi hủy ca");
    }
  };
  ...
  const handleCancelRecurringShift = async () => {
    ...
    try {
      await api.schedules.cancelShift(selectedShift.id, "series", selectedShiftDate);
      const updatedShifts = await api.schedules.getMyShifts();
      onUpdateShifts(updatedShifts);
      setSelectedShift(null);
      onShowToast(
      ...
      onShowToast(err.message || "Lỗi khi hủy chuỗi ca");
    }
  };
