...
describe('i18n Translation System', () => {
  describe('translate() in English mode', () => {
    ...
    it('returns English strings from nested domain modules', () => {
      ...
      expect(translate('Tiếng Anh', 'auth.login_subtitle')).toBe(
        'Log in to continue to the system',
      );
      expect(translate('Tiếng Anh', 'auth.org_name')).toBe(
        'Academy of Military Science and Technology',
      );
      expect(translate('Tiếng Anh', 'auth.logo_alt')).toBe(
        'Academy of Military Science and Technology Logo',
      );
      expect(translate('Tiếng Anh', 'auth.account_pending_approval')).toBe(
  ...
  describe('translate() in Vietnamese mode', () => {
    ...
    it('returns Vietnamese strings from nested domain modules', () => {
      ...
      expect(translate('Tiếng Việt', 'auth.login_subtitle')).toBe(
        'Đăng nhập để tiếp tục vào hệ thống',
      );
      expect(translate('Tiếng Việt', 'auth.account_pending_approval')).toBe(
        'Tài khoản đang được chờ duyệt',
      );
  ...
  describe('Parameter interpolation', () => {
    it('replaces {{param}} in translations', () => {
      const enResult = translate('Tiếng Anh', 'accounts.view_profile_title', {
        name: 'Nguyen Van A',
      });
      expect(enResult).toBe('View detailed profile of Nguyen Van A');

      const viResult = translate('Tiếng Việt', 'accounts.view_profile_title', {
        name: 'Nguyễn Văn A',
      });
    ...
    it('replaces {param} single brace format', () => {
      const result = translate('Tiếng Anh', 'Welcome {name} to {place}', {
        name: 'Alice',
        place: 'Office',
      });
