...
| **2.1** | Đăng ký / cập nhật lịch CTV | [04-dang-ky-cap-nhat-lich-lam-viec.md](sequence-diagrams/04-dang-ky-cap-nhat-lich-lam-viec.md) | `GET /api/v1/users/me/schedule`<br/>`PUT /api/v1/users/me/schedule` | `schedule.controller.ts:getMySchedule`<br/>`schedule.controller.ts:putMySchedule` | `schedule.query.service.ts:getMySchedule`<br/>`schedule.command.service.ts:upsertSchedule`<br/>*(Facade: `schedule.service.ts`)* |
| **2.2** | Xem lịch tuần cá nhân (CTV) | [11-xem-lich-tuan-va-lich-su.md](sequence-diagrams/11-xem-lich-tuan-va-lich-su.md) | `GET /api/v1/users/me/schedule` | `schedule.controller.ts:getMySchedule` | `schedule.query.service.ts:getMySchedule`<br/>*(Facade: `schedule.service.ts`)* |
| **2.2** | Xem lịch sử làm việc (CTV) | [11-xem-lich-tuan-va-lich-su.md](sequence-diagrams/11-xem-lich-tuan-va-lich-su.md) | `GET /api/v1/users/me/work-history` | `schedule.controller.ts:getMyWorkHistory` | `work-history.service.ts:getMyWorkHistory`<br/>*(Facade: `schedule.service.ts`)* |
| **2.3** | Chốt lịch sử tự động 17:30 | [06-chot-lich-su-lam-viec-tu-dong.md](sequence-diagrams/06-chot-lich-su-lam-viec-tu-dong.md) | Cron / Startup Recovery (Không qua HTTP) | `jobs/schedule-snapshot.job.ts:startScheduleSnapshotJob` | `work-history.service.ts:snapshotTodayWorkHistory`<br/>*(Facade: `schedule.service.ts`)* |
| **2.4** | Xem lịch tuần tổng hợp (Admin) | [07-xem-lich-lam-viec-tong-hop.md](sequence-diagrams/07-xem-lich-lam-viec-tong-hop.md) | `GET /api/v1/schedule/weekly-summary` | `schedule.controller.ts:getWeeklySummary` | `schedule.query.service.ts:getWeeklySummary`<br/>*(Facade: `schedule.service.ts`)* |
| **2.4** | Xem lịch sử tổng hợp (Admin) | [07-xem-lich-lam-viec-tong-hop.md](sequence-diagrams/07-xem-lich-lam-viec-tong-hop.md) | `GET /api/v1/work-history` | `schedule.controller.ts:getWorkHistory` | `work-history.service.ts:getWorkHistory`<br/>*(Facade: `schedule.service.ts`)* |
...
| **2.5** | Xem lịch tuần CTV trong modal | [12-xem-chi-tiet-ca-va-ho-so-ctv.md](sequence-diagrams/12-xem-chi-tiet-ca-va-ho-so-ctv.md) | `GET /api/v1/accounts/:id/schedule` | `schedule.controller.ts:getAccountSchedule` | `schedule.query.service.ts:getAccountSchedule`<br/>*(Facade: `schedule.service.ts`)* |
| **2.5** | Xem lịch sử CTV trong modal | [12-xem-chi-tiet-ca-va-ho-so-ctv.md](sequence-diagrams/12-xem-chi-tiet-ca-va-ho-so-ctv.md) | `GET /api/v1/work-history?accountId=:id` | `schedule.controller.ts:getWorkHistory` | `work-history.service.ts:getWorkHistory`<br/>*(Facade: `schedule.service.ts`)* |
...
| **`Schedule`** | Lưu cấu hình lịch tuần lặp lại cố định của CTV (quan hệ 1:1 với `Account`), phòng trực (`roomCode`), phiên bản (`version`) | `schedule.command.service.ts`<br/>`schedule.query.service.ts`<br/>*(Facade: `schedule.service.ts`)* | `GET /users/me/schedule`<br/>`PUT /users/me/schedule`<br/>`GET /accounts/:id/schedule`<br/>`GET /schedule/weekly-summary`<br/>Background Snapshot (17:30 Bangkok) |
| **`Shift`** | Lưu chi tiết các ca trực cố định trong tuần của một `Schedule`: thứ (`weekday`: 1..5) và buổi (`period`: `MORNING`/`AFTERNOON`). Khóa chính kết hợp `(scheduleId, weekday, period)` | `schedule.command.service.ts`<br/>`schedule.query.service.ts`<br/>`work-history.service.ts`<br/>*(Facade: `schedule.service.ts`)* | `GET /users/me/schedule`<br/>`PUT /users/me/schedule`<br/>`GET /accounts/:id/schedule`<br/>`GET /schedule/weekly-summary`<br/>Background Snapshot (17:30 Bangkok) |
| **`History`** | Lưu ảnh chụp bất biến các ca làm việc đã hoàn thành (`status = 'COMPLETED'`) được chốt tự động vào 17:30 hằng ngày. Ràng buộc duy nhất `@@unique([accountId, workDate, period])` | `work-history.service.ts`<br/>*(Facade: `schedule.service.ts`)* | `GET /users/me/work-history`<br/>`GET /work-history`<br/>Background Snapshot (17:30 Bangkok & Startup Recovery) |
